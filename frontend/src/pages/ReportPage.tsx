      ...
      <div className="max-w-4xl mx-auto px-4">
        <div className="bg-white rounded-lg shadow-lg p-8">
          ...
          <div className="space-y-4">
            <h2 className="text-xl font-semibold text-gray-800">接收到的訊息：</h2>
            {messages.length === 0 ? (
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex-1">
                        <div className="text-sm font-medium text-gray-700">
                          ...
                          {message.type === 'scraping_completed' && '✅ 執行完成'}
                          {message.type === 'scraping_error' && '❌ 執行失敗'}
                          {message.type === 'connection_established' && '🔗 連接建立'}
                          {message.type === 'raw' && '📨 訊息'}
                        </div>
                        <div className="text-gray-800 mt-1">{message.message}</div>
                        {message.error && (
                          <div className="text-red-600 text-sm mt-1">錯誤: {message.error}</div>
                        )}
                      </div>
